  ...
  <div class="container">
    ...
    <div class="grid ag-theme-quartz" ref="agGridRef">
      <ag-grid-vue :class="['aggrid_Root']" @grid-ready="onGridReady" :rowSelection="rowSelection"
        ...
        :columnDefs="columnDefs" :rowGroupPanelShow="rowGroupPanelShow" :defaultColDef="defaultColDef"
        :enableRangeSelection="enableRangeSelection" :suppressAggFuncInHeader="suppressAggFuncInHeader"
        :groupDefaultExpanded="groupDefaultExpanded" :suppressRowClickSelection="suppressRowClickSelection"
        :localeText="AG_GRID_LOCALE_CN" @rowSelected="HandleSeleChange" @columnMoved="onColumnChanged"
        :treeData="true" :getDataPath="getDataPath"
        @columnResized="onColumnChanged" @columnVisible="onColumnChanged" @onColumnPinned="onColumnChanged"
        @sort-changed="onFilterChanged" @filter-changed="onFilterChanged" @sortChanged="onColumnChanged"
        @cellClicked="HandleCellEdit" v-bind="$attrs">
      </ag-grid-vue>
...
const getDataPath = function(data) {
    // 通过节点的 id 和父节点的关系返回路径
    return data.children ? [data.deptName] : [data.deptName,data.id];
  }
